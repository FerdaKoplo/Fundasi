...
import TokenClaim from "../../../components/token/token-claim";

const UserProfile = () => {
  const [activeTab, setActiveTab] = useState<
    "about" | "campaigns" | "trustpoint" | "Claim Token"
  >("about");
  const { user } = useAuth();
      ...
      <div className="min-h-screen  text-white flex flex-col items-center ">
        <div className="flex flex-col items-center pt-[30px]">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {user?.campaignCount?.toString() ?? "0"} Campaign Project
        ...
        <div className="relative mt-2 flex flex-col gap-5 text-base w-screen px-32 p-6">
          ...
          {activeTab === "trustpoint" && (
            <div className="p-10 w-full flex flex-col items-center justify-center px-4 text-gray-300 text-sm ">
              <div className="flex items-center gap-10">
                <TrustPointChart
                  value={Number(user?.trustPoints ?? BigInt(0))}
                  total={100}
                />
                <div className="">
                  <p className="font-medium text-xl">
                    {Number(user?.trustPoints ?? BigInt(0))} Trust Points
                  </p>
          ...
          {activeTab === "Claim Token" && (
            <div className="p-10 w-full flex flex-col items-center justify-center px-4 text-gray-300 text-sm ">
              <h2 className="text-xl font-semibold mb-4">
                🎁 Claim Your Free Token
              </h2>
              <p className="text-center mb-4">
                Kamu bisa mengklaim token gratis sebagai modal awal untuk
                mendukung kampanye.
              </p>
              <TokenClaim />
            </div>
